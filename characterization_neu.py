...
def get_min_values(measure_table, col_of_interest, plot_filename, initial_guess=None):
    ...
    sigma = None
    popt, pcov = curve_fit(sin2_func_in_degrees, measure_table['position'], measure_table[col_of_interest], p0=initial_guess, sigma=sigma, maxfev = 10000)
...
if __name__ == "__main__":
    ...
    power_meter_ref_value = np.mean(measures_power_meter_ref)
    power_meter_path2_value = np.mean(measures_power_meter_path2)
    faktor = power_meter_path2_value / power_meter_ref_value
    ...
    measure_table = do_measurement(start=0, stop=360, step_size=1, motor=motor1,power_meter_ref=power_meter_ref, power_meter_path1=power_meter_path1, power_meter_path2=power_meter_path2, file_name=f"{timestamp}_design_orientation.csv")
    ...
    for _ in range(10):
        lib.TLPMX_measPower(power_meter_path2, byref(power), 1)
        measures_power_meter_path2.append(power.value)
    power_do = np.mean(measures_power_meter_path2)
    ...
    for _ in range(10):
        ...
        lib.TLPMX_measPower(power_meter_path2, byref(power), 1)
        measures_power_meter_path2.append(power.value)
    power_meter_ref_value1 = np.mean(measures_power_meter_ref)
    power_meter_path2_value1 = np.mean(measures_power_meter_path2)
    iso_do1 = (power_meter_path2_value1 / (power_meter_ref_value1 * faktor))
    ...
    for _ in range(10):
        ...
        lib.TLPMX_measPower(power_meter_path2, byref(power), 1)
        measures_power_meter_path2.append(power.value)
    power_meter_ref_value2 = np.mean(measures_power_meter_ref)
    power_meter_path2_value2 = np.mean(measures_power_meter_path2)
    iso_do2 = (power_meter_path2_value / (power_meter_ref_value2 * faktor))
    iso_do = max(iso_do1, iso_do2)
    isolation_do = 10 * np.log10(iso_do)
    logger.info(f"Iso design orientation: {iso_do}, iso1: {iso_do1}, iso2:{iso_do2} ,Iso design orientation: {isolation_do} , Power ref do1:{power_meter_ref_value1},  Power ref do2:{power_meter_ref_value2}")
    ...
    logging.info(f"insertion loss1: {insertion_loss1}, insertion loss1: {insertion_loss2}, pref1 = {power_meter_ref_value1},pref2 = {power_meter_ref_value2}, power2_1 = {power_meter_path2_value1}, power2_2 = {power_meter_path2_value2}")
    ...
    transmission_design = max(transmission_design1, transmission_design2)
    print(f"Transmission at design orientation: {transmission_design}")
    logging.info(f"Transmission at design orientation: {transmission_design}, Power ref: {power_meter_ref_value}")
    ...
    if power_meter_path2_value1 > power_meter_path2_value2:
        power_meter_path2_value = power_meter_path2_value1
        design_oriantation = min_fit_do_pos+90
    else:
        power_meter_path2_value = power_meter_path2_value2
    ...
    per = 10 * np.log(max(max_values_per.values())/min(min_values_per.values()))
